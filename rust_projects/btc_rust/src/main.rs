fn main() {
    let version = read_version("");
    println!("Version: {}", version);
}

fn read_version(tx_hex: &str) ->u32 {
    let tx_bytes = hex::decode(tx_hex).unwrap();
    let version_bytes: [u8; 4] = (&tx_bytes[0..4]).try_into().unwrap();
    u32::from_le_bytes(version_bytes)
    // let version_bytes = &tx_bytes[0..4];
    // println!("version bytes: {:?}", version_bytes);
    // 1
}

// https://github.com/bitcoin-dev-project/bitcoiner-intro-to-rust/blob/main/08_traits_and_reading_bytes.md