...
// Debug function
pub fn aoc02_p1_debug(input: &str) -> Vec<String> {
    input
        .lines()
        .map(|line| {
            let moves: Vec<Move> = line
            ...
            let score = match moves[0].partial_cmp(&moves[1]) {
                Some(Ordering::Equal) => {3 + moves[1] as u32}
                Some(Ordering::Less) => {6 + moves[1] as u32}
                Some(Ordering::Greater) => {0 + moves[1] as u32}
                None => {panic!("moves should be comparable")}
...
pub fn aoc02_p2_debug(input: &str) -> Vec<String> {
    input
        .lines()
        .map(|line| {
            let (opponent_str, outcome_code) = line
            ...
            let score = score_bonus + our_move as u32;
            
            format!(
                "Line: {:<5} | Opponent: {:?}, Our Move: {:?}, Outcome Code: {}, Score: {}",
                line, opponent_move, our_move, outcome_code, score
            )
        })
        .collect()
}
